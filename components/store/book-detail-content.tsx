...
export function BookDetailContent({ book, relatedBooks }: { book: any; relatedBooks: any[] }) {
    ...
    async function checkPurchase() {
      ...
      if (user) {
        const { data: purchase } = await supabase
          ...
          .single()
        setHasPurchased(!!purchase || book.price === 0)
      }
    }
    checkPurchase()
                ...
                <Link key={relBook.id} href={`/store/${relBook.id}`}>
                  <Card className="overflow-hidden hover:shadow-lg transition-shadow">
                    <div className="aspect-[3/4] bg-muted">
                      <img
                        src={relBook.cover_url || "/placeholder.svg?height=600&width=450&query=law+book+cover"}
                        alt={relBook.title}
                        className="h-full w-full object-cover"
                      />
                    </div>
